...
class OW_Dispatcher
{
     ...
     * @var array
     */
    private $catchAllRequestsAttributes = [];
    /**
     * @var array
     */
    ...
    public function addCatchAllRequestsExclude($key, $controller, $action = null)
    {
        ...
        $this->catchAllRequestsExcludes[$key][] = [self::CATCH_ALL_REQUEST_KEY_CTRL => $controller, self::CATCH_ALL_REQUEST_KEY_ACTION => $action];
    }
    ...
    private function processCatchAllRequestsAttrs()
    {
        ...
        foreach ($this->catchAllRequestsExcludes[$lastKey] as $exclude) {
            if ($this->dispatchAttributes[self::ATTRS_KEY_CTRL] === $exclude[self::CATCH_ALL_REQUEST_KEY_CTRL] &&
                ($exclude[self::CATCH_ALL_REQUEST_KEY_ACTION] === null || $this->dispatchAttributes[self::ATTRS_KEY_ACTION] === $exclude[self::CATCH_ALL_REQUEST_KEY_ACTION])) {
                $catchRequest = false;
                break;
            }
        }
        if ($catchRequest) {
            if (isset($this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_REDIRECT]) && (bool)$this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_REDIRECT]) {
                $route =
                    isset($this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_ROUTE]) ? trim($this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_ROUTE]) : null;
                ...
                $redirectUrl = ($route !== null) ?
                    ...
                    OW::getRouter()
                      ->urlFor($this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_CTRL], $this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_ACTION], $params);

                $redirectUrl = OW::getRequest()->buildUrlQueryString($redirectUrl, ['back_uri' => OW::getRequest()->getRequestUri()]);
                ...
                if (isset($this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_JS]) && (bool)$this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_JS]) {
                    ...
                    if (array_key_exists('base.members_only', $this->catchAllRequestsAttributes)) {
                        if ($this->dispatchAttributes[self::CATCH_ALL_REQUEST_KEY_CTRL] === 'BASE_CTRL_User' && $this->dispatchAttributes[self::CATCH_ALL_REQUEST_KEY_ACTION] === 'standardSignIn') {
                            ...
                            OW::getDocument()->addOnloadScript("window.location = '" . OW::getRequest()->buildUrlQueryString($redirectUrl, ['back_uri' => $backUri]) . "'");
                            return null;
                        }

                        $ru = OW::getRequest()->buildUrlQueryString(OW::getRouter()->urlForRoute('static_sign_in'), ['back_uri' => OW::getRequest()->getRequestUri()]);
