...
class BASE_CTRL_Rate extends OW_ActionController
{
    ...
    public function updateRate()
    {
        ...
        {
            exit(json_encode(['errorMessage' => 'Invalid request']));
        }
        ...
        {
            exit(json_encode(['errorMessage' => OW::getLanguage()->text('base', 'rate_cmp_auth_error_message')]));
        }

        if ( $userId === $ownerId )
        {
            exit(json_encode(['errorMessage' => OW::getLanguage()->text('base', 'rate_cmp_owner_cant_rate_error_message')]));
        }

        if ( false )
        {
            //TODO add authorization error
            exit(json_encode(['errorMessage' => 'Auth error']));
        }

        if ( BOL_UserService::getInstance()->isBlocked(OW::getUser()->getId(), $ownerId) )
        {
            exit(json_encode(['errorMessage' => OW::getLanguage()->text('base', 'user_block_message')]));
        }
        ...
        exit(json_encode(['totalScoreCmp' => $totalScoreCmp->render(), 'message' => OW::getLanguage()->text('base', 'rate_cmp_success_message')]));
    }
