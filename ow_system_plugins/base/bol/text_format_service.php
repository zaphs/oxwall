...
final class BOL_TextFormatService
{
    ...
    {
        $this->buttonTags = [
            self::WS_BTN_BOLD => ['tags' => ['b', 'strong', 'span'], 'attrs' => ['span.style']],
            self::WS_BTN_ITALIC => ['tags' => ['i', 'em', 'span'], 'attrs' => ['span.style']],
            self::WS_BTN_UNDERLINE => ['tags' => ['u', 'span'], 'attrs' => ['span.style']],
            self::WS_BTN_ORDERED_LIST => ['tags' => ['ol', 'li'], 'attrs' => []],
            self::WS_BTN_UNORDERED_LIST => ['tags' => ['ul', 'li'], 'attrs' => []],
            self::WS_BTN_LINK => ['tags' => ['a'], 'attrs' => ['a.href', 'a.title', 'a.target', 'a.rel']],
            self::WS_BTN_IMAGE => ['tags' => ['img', 'a'], 'attrs' => ['img.src', 'img.width', 'img.height', 'img.alt', 'img.style', 'a.href', 'a.target']],
        ];

        $this->videoParams = [
            'tags' => ['object', 'iframe', 'param', 'embed'],
            'attrs' => ['id', 'width', 'height', 'data', 'type', 'name', 'value', 'src', 'allowfullscreen', 'frameborder', 'flashvars']
        ];
    ...
    {
        if ( $type !== null && in_array($type, ['tags', 'attrs']) )
        {
    ...
    public function processWsForOutput( $text, array $params = [])
    {
        //hotfix for Chrome
...
//        $text = str_replace(array('<div>', '</div>'), array('', '<br />'), $text);
        ...
        //hotfix for ie8
        $text = str_ireplace(['<SPAN class=ow_ws_video>'], '<span class="ow_ws_video">', $text);
        ...
        // default tags list
        $tagsArray = ['br', 'span', 'blockquote', 'p'];
        // default attrs list
        $attrsArray = ['class', 'p.style'];
    ...
    public function processWsForInput( $text, array $params = [])
    {
        //printVar($text);
...
//        }
    ...
    private function processHtml( $text )
    {
        ...
        return ['text' => $text, 'search' => $searchArray, 'replace' => $replaceArray];
    }
    ...
    public static function addVideoCodeParam( $code, $name = 'wmode', $value = 'transparent' )
    {
        ...
        if ( preg_match("/<iframe[^>]*src=['\"]([^'\"]+)['\"]/i", !empty($repl) ? $repl : $code, $matches) )
        {
            ...
            {
                $src = "http:" . $matches[1];
                $src = OW::getRequest()->buildUrlQueryString($src, [$name => $value]);
                $src = substr($src, 5);
            }
            else
            {
                $src = OW::getRequest()->buildUrlQueryString($matches[1], [$name => $value]);
            }
